...
<<<<<<< HEAD
  ...
  if (updated) {
    ...
    while (!timeClient.update()) {
      ...
      if (millis() - startAttemptTime > timeout) {  // Verifica se o tempo limite foi excedido
...
void loop() {
  ...
  if ((currentMillis - lastConectionTest >= 380000) && !isSpraying) {  //change again to 380000
...
<<<<<<< HEAD
  ...
  if (greenhouseOn) {  // Double check on the rele, but it should always be off to get here
